...
cd "C:\Users\steve\Desktop\EC_SDSA"
...
pagecreate(page=annualtest_{%pays}) a %startsim0m1 %endsmpl
	copy quarterly_{%pays}\maturity_{%pays}
	copy quarterly_{%pays}\nsim
	matrix M = @zeros(1, 5)
	matrix(nsim,5) matrix_test
...
'4) Annualize the shocks
	...
	stom(acc_eps_{%var},acc_{%var} )
		...
		wend
	next
	pageselect annualtest_{%pays}
	'copy quarterly_{%pays}\ann_ltn_10yy 
	copy(c=s) quarterly_{%pays}\eps_ltn_10y
	smpl %endate+1 %endsim
	series test = @cumsum(eps_ltn_10y)
	!k= 0
	while !k<=4
	smpl %startsim1+!k  %startsim1+!k
	series testi=test*(!k+1)/maturity_{%pays} 	
	!k=!k+1
 	wend
	smpl %endate+1 %endsim
	stom( testi, testv )
	M=@vcat(M, testv)
	for !i = 1 to 5
   	matrix matrix_test(!j,!i) = M(!j+1,!i)
	matrix  ann_ltn_10y = matrix_test 
	next
	pageselect quarterly_{%pays}
	!j=!j+1
wend
...
pagecreate(page=annual_{%pays}) a %startsim0m1 %endsmpl
		...
		copy(c=s) quarterly_{%pays}\ann_soldep_p
		copy(c=s) quarterly_{%pays}\ann_stn_3m      
		copy(c=s) annualtest_{%pays}\ann_ltn_10y 
		copy(c=s) quarterly_{%pays}\ann_g_v_yoy
		copy(c=s) quarterly_{%pays}\nsim
wfopen %wf0 
...
stom(g_base,base_m)
stom( dda_bkcom_000_{%pays}, dda_bkcom_vec_000_{%pays} )
...
while !j<=nsim
 			!k=1 'projection year
			while !k<=5
				sim_soldep_p(!j,!k) = base_m(!k+1,1)+ann_soldep_p(!j,!k) 
				sim_stn_3m(!j,!k) = base_m(!k+1,2)+ann_stn_3m(!j,!k)
				sim_ltn_10y(!j,!k) = base_m(!k+1,3)+ann_ltn_10y(!j,!k)
				sim_g_v_yoy(!j,!k) = base_m(!k+1,4)+ann_g_v_yoy(!j,!k) 
				sim_tx_moy(!j,!k) =  (base_m(!k+1,5)+alphalt_{%pays}*ann_ltn_10y(!j,!k)+alphact_{%pays}*ann_stn_3m(!j,!k))*(base_m(!k+1,5)+alphalt_{%pays}*ann_ltn_10y(!j,!k)+alphact_{%pays}*ann_stn_3m(!j,!k)>0) 'positivity constraint
			!k=!k+1
			wend
	!j=!j+1
wend
...
while !j<=nsim
	sim_dette_iir(!j,1)=dettem1*((1+sim_tx_moy(!j,1))/(1+sim_g_v_yoy(!j,1))) -sim_soldep_p(!j,1) 
 			!k=2 'projection year
			while !k<=5
			sim_dette_iir(!j,!k)=sim_dette_iir(!j,!k-1)*((1+sim_tx_moy(!j,!k))/(1+sim_g_v_yoy(!j,!k))) -sim_soldep_p(!j,!k) +dda_bkcom_vec_000_{%pays}(!k)/100
